// useMember — membership state + join flow (specs 01/04).
//
//   const { status, member, join, error, loading } = useMember();
//
...
import { auth, db, ensureSignedIn, TRIP_ID } from '../firebase.js';

const STORAGE_KEY = 'japan2026.join';
...
export function useMember() {
  ...
  // Track auth without forcing a sign-in (poster visitors stay signed out;
  // anonymous sign-in happens inside join via ensureSignedIn).
  ...
  const writeJoin = useCallback(async (inviteToken, name) => {
    ...
    await setDoc(doc(db, 'trips', TRIP_ID, 'members', user.uid), {
      name: trimmed,
      inviteToken,
  ...
  useEffect(() => {
    ...
    setChecking(true);
    return onSnapshot(
      doc(db, 'trips', TRIP_ID, 'members', uid),
      (snap) => {
        if (snap.exists()) {
          wasMember.current = true;
